...
const scopes =
  "user-read-private user-top-read playlist-modify-public user-read-recently-played playlist-read-private";
let getLoginUrl = async (state: string) => {
  let response = await fetch(
    ...
    }&state=${state}&scope=${encodeURIComponent(scopes)}&show_dialog=true`
  );
  return response.url;
};
